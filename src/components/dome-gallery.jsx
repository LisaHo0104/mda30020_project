...
function buildItems(pool, seg) {
  ...
  const normalizedImages = pool.map(normalizeImage);
...
/**
 * @typedef {Object} DomeGalleryProps
 * @property {(string | ({ src: string, alt?: string } & Record<string, unknown>))[]=} images
 * @property {number=} fit
 * @property {"auto" | "min" | "max" | "width" | "height"=} fitBasis
 * @property {number=} minRadius
 * @property {number=} maxRadius
 * @property {number=} padFactor
 * @property {string=} overlayBlurColor
 * @property {number=} maxVerticalRotationDeg
 * @property {number=} dragSensitivity
 * @property {number=} enlargeTransitionMs
 * @property {number=} segments
 * @property {number=} dragDampening
 * @property {string=} openedImageWidth
 * @property {string=} openedImageHeight
...
export default function DomeGallery({
  ...
  const openItemFromElement = useCallback(
    (el) => {
      ...
      lockScroll();
      const parent = el.parentElement;
      const itemIndex = Number(parent.dataset.index ?? 0);
      const item = items[itemIndex];
      onImageOpen?.(item, itemIndex);
      ...
      const openedContent = renderOpenedContent?.(item, itemIndex, rawSrc);
      if (openedContent instanceof Node) {
        overlay.appendChild(openedContent);
      } else {
